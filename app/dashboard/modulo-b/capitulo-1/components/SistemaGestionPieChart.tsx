// app/dashboard/modulo-b/components/SistemaGestionPieChart.tsx
"use client";
...
import { formatPercent } from '../data/dataProcessor'

interface SistemaGestionPieChartProps {
...
const SistemaGestionPieChart: React.FC<SistemaGestionPieChartProps> = ({
  data,
  titulo,
  descripcion,
  gestion,
  ...
  const chartOptions: ApexOptions = {
    ...
    dataLabels: {
      ...
      formatter: function (val: number, opts) {
        ...
        return [seriesName, formatPercent(val)]
      },
      style: {
        fontSize: '20px',
        fontWeight: 600,
    ...
    tooltip: {
      ...
      y: {
        formatter: function (val: number) {
          return `${val} empresas (${formatPercent((val / data.series.reduce((a, b) => a + b, 0)) * 100)})`
        }
      }
    },
    responsive: [{
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-all duration-300">
      ...
      <Chart
        ...
        series={chartSeries}
        type="donut"
        height={350} // Reducimos un poco la altura porque el título ahora está fuera
      />
    </div>
  )
}

export default SistemaGestionPieChart